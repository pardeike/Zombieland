...
- Feeding consumes one valid organic non-Zombieland corpse and adds every advertised growth pulse. A corpse is not offered when its full gain would exceed the configured cell limit. Mechanoid and other non-flesh corpses are not valid feed.
...
- Occupancy work is proportional to the bounded logical footprint. Migration membership uses a transient hash set, and one movement pulse performs at most one queued repair in addition to ordinary movement. Source-room connectivity uses a transient map/root/shape cache: a stable movement pulse scans the footprint once to find room articulation cells, then evaluates every source candidate from that cache.
...
Performance evidence from the implementation pass showed default 400-cell and diagnostic 4000-cell Symbiants close to same-session no-Symbiant baselines after cache, rendering, and hot-path fixes. Rerun those stress checks when touching rendering, path cost, cell stat effects, movement connectivity or geometry caches, Symbiant ticking, host hediff sync, or active-cache behavior.
