...
namespace ZombieLand
{
	class Patches
	{
		...
		{
			static void Prefix(IAttackTargetSearcher searcher)
			{
				var pawn = searcher as Pawn;
				var zombie = pawn?.LastAttackedTarget.Thing as Zombie;
				if (zombie != null && zombie.Downed)
					pawn.mindState.lastAttackedTarget = LocalTargetInfo.Invalid;
			}

			static void Postfix(ref List<Pair<IAttackTarget, float>> __result)
			{
				if (__result.Count == 1)
				{
					var singleZombie = __result[0].First.Thing as Zombie;
					if (singleZombie != null && singleZombie.Downed)
						return;
				}

				__result = __result.Where(pair =>
					{
						var zombie = pair.First as Zombie;
						if (zombie == null) return true;
						return zombie.Downed == false;
					}).ToList();
			}
		}
