...
namespace fastJSON
{
	...
	internal sealed class JsonParser
	{
		...
		int index;
		readonly bool allownonquotedkey = false;
		readonly int _len = 0;
		...
		{
			var table = new Dictionary<string, object>(10);
		...
		private unsafe object ParseNumber(char* p)
		{
			...
			if (dob)
			{
				var s = UnsafeSubstring(p, startIndex, index - startIndex);// json.Substring(startIndex, index - startIndex);
