	...
	static class Pawn_DrawTracker_Constructor_Patch
	{
		...
		static void Postfix(Pawn_DrawTracker __instance, Pawn pawn)
		{
			if (__instance.leaner?.GetType() != typeof(PawnLeaner))
			{
				...
				if (patches != null)
				{
					var postfixes = patches.Postfixes.Where(p => p.owner != "net.pardeike.zombieland");
					var transpilers = patches.Transpilers.Where(p => p.owner != "net.pardeike.zombieland");
					mods = postfixes.Union(transpilers).Join(t => t.owner);
				}
